...
class MonitoringScreen extends StatefulWidget {
  ...
  MonitoringScreen({super.key, this.initialLandId});
...
class _MonitoringScreenState extends State<MonitoringScreen> with SingleTickerProviderStateMixin {
  ...
  Widget? _getFloatingActionButton() {
    if (_tabController.index == 1) {
      return FloatingActionButton.extended(
        ...
        label: Text('Buat Tugas', style: TextStyle(color: Colors.white)),
      );
    }
    ...
    return SizedBox(
      ...
      child: FlutterMap(
        ...
        children: [
          ...
          MarkerLayer(markers: [Marker(point: LatLng(lat, lng), width: 40, height: 40, child: Icon(Icons.location_on, color: Colors.red, size: 40))]),
            ...
            child: Column(
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(color: Color(0xFF0A3D2F), borderRadius: BorderRadius.circular(4)),
                  child: Column(
                    children: [
                      Text('${_selectedLand?.areaSize ?? 0} ha', style: TextStyle(color: context.cardBg, fontSize: 10)),
                      Text(_selectedLand?.name ?? 'Tanaman', style: TextStyle(color: context.cardBg, fontWeight: FontWeight.bold)),
                    ],
                  ),
                )
              ],
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<Land>(
          ...
          icon: Icon(Icons.keyboard_arrow_down, color: context.bgGrey),
          items: _userLands.map((Land land) {
            return DropdownMenuItem<Land>(
              ...
              child: Column(
                ...
                children: [
                  Text(land.name, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: context.textPrimary)),
                  Text(land.location ?? 'Lokasi tidak diketahui', style: TextStyle(color: context.bgGrey, fontSize: 12)),
                ],
              ),
            );
          }).toList(),
  ...
  Widget _buildTabButton(String text, int index) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isSelected ? Color(0xFF0A3D2F) : Color(0xFFE8F5E9),
          borderRadius: BorderRadius.circular(20),
        ),
        child: Center(
          child: Text(text, style: TextStyle(color: isSelected ? context.cardBg : context.textPrimary, fontWeight: FontWeight.bold, fontSize: 13)),
        ),
      ),
    );
  }
  ...
  Widget _buildTabContent() {
    switch (_tabController.index) {
      ...
      case 2: return Center(child: Padding(padding: EdgeInsets.all(32), child: Text("Integrasi API coming soon")));
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 24),
          Text('Grafik Lahan (Manual Input)', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          SizedBox(height: 12),
  ...
  Widget _buildWeatherCard(dynamic temp) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [context.cardBg, Colors.green[50]!]),
        borderRadius: BorderRadius.circular(12),
      ...
      child: Row(
        children: [
          Column(
            children: [
              Icon(Icons.wb_sunny_rounded, color: Colors.orange, size: 36),
              SizedBox(height: 8),
              Text('$temp°C', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20)),
              Text('Hari ini', style: TextStyle(fontSize: 10, color: context.bgGrey)),
            ],
          ),
          Container(width: 1, height: 50, color: context.dividerColor, margin: EdgeInsets.symmetric(horizontal: 20)),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Lokasi Lahan', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                Text(_displayAddress, style: TextStyle(fontSize: 11, color: context.textPrimary), maxLines: 2, overflow: TextOverflow.ellipsis),
                SizedBox(height: 4),
                Text('${_selectedLand?.latitude?.toStringAsFixed(4)}, ${_selectedLand?.longitude?.toStringAsFixed(4)}', style: TextStyle(fontSize: 9, color: context.bgGrey)),
              ],
            ),
          )
        ],
  ...
  Widget _buildChartContainer({required String title, required List<double> data, required Color color, required VoidCallback onUpdate}) {
    return Container(
      padding: EdgeInsets.all(16),
      decoration: BoxDecoration(color: context.cardBg, borderRadius: BorderRadius.circular(12), border: Border.all(color: context.dividerColor)),
      child: Column(
        children: [
          Row(
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: onUpdate,
                  style: ElevatedButton.styleFrom(backgroundColor: color, padding: EdgeInsets.symmetric(horizontal: 12)),
                  child: Text("Input Data", style: TextStyle(color: context.cardBg, fontSize: 10)),
                ),
              )
            ],
          ),
            ...
            children: List.generate(7, (index) {
              ...
              return Column(
                children: [
                  ...
                  SizedBox(height: 4),
                  Text(index < data.length ? val.toStringAsFixed(0) : '-', style: TextStyle(fontSize: 10)),
                ],
              );
            }),
          )
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextFormField(controller: controller, keyboardType: TextInputType.number, decoration: InputDecoration(labelText: isWater ? 'Jumlah Air (Liter)' : 'Tinggi Tanaman (cm)', border: OutlineInputBorder())),
            SizedBox(height: 10),
            Text('Data akan dicatat untuk hari ini.', style: TextStyle(fontSize: 12, color: context.textMuted)),
          ],
        ),
        actions: [
    ...
    return Padding(
      ...
      child: Column(
        children: [
          Container(
            padding: EdgeInsets.all(16),
            decoration: BoxDecoration(color: context.cardBg, borderRadius: BorderRadius.circular(16), border: Border.all(color: context.dividerColor)),
          ...
          SizedBox(height: 24),
          Align(alignment: Alignment.centerLeft, child: Text('Tugas: ${DateFormat('dd MMM yyyy').format(_selectedTaskDate)}', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16))),
          SizedBox(height: 12),
          if (_dailyTasks.isEmpty) Padding(padding: EdgeInsets.all(20), child: Text("Tidak ada tugas hari ini.", style: TextStyle(color: context.textMuted))),
          ListView.builder(
            shrinkWrap: true,
            ...
            itemBuilder: (context, index) {
              ...
              return Dismissible(
                ...
                background: Container(color: Colors.red, alignment: Alignment.centerRight, padding: EdgeInsets.only(right: 20), child: Icon(Icons.delete, color: context.cardBg)),
                onDismissed: (_) async { await _landService.deleteTask(task.id); _loadData(); },
                child: Card(
                  elevation: 0,
                  shape: RoundedRectangleBorder(side: BorderSide(color: context.dividerColor), borderRadius: BorderRadius.circular(8)),
                  child: CheckboxListTile(
                    title: Text(task.title, style: TextStyle(decoration: task.isCompleted ? TextDecoration.lineThrough : null)),
                    subtitle: Text('${task.description ?? ''} • ${DateFormat('HH:mm').format(task.dueDate)}', style: TextStyle(fontSize: 12)),
        ...
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                TextField(controller: titleCtrl, decoration: InputDecoration(labelText: 'Judul Tugas')),
                TextField(controller: descCtrl, decoration: InputDecoration(labelText: 'Deskripsi')),
                SizedBox(height: 16),
                Row(
                  children: [
  ...
  Widget _buildSettingsTab() {
    if (_selectedLand == null) return Center(child: Text("Pilih lahan terlebih dahulu"));
    return Padding(
      padding: EdgeInsets.all(16.0),
      child: Card(
        elevation: 0,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16), side: BorderSide(color: context.dividerColor)),
        child: Padding(
          padding: EdgeInsets.all(16.0),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text("Edit Informasi Lahan", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              Divider(),
              SizedBox(height: 10),
              ...
              SizedBox(height: 10),
              Text("Data Finansial", style: TextStyle(fontWeight: FontWeight.bold, color: context.primaryColor)),
              SizedBox(height: 10),
              Row(children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(backgroundColor: context.primaryColor, padding: EdgeInsets.symmetric(vertical: 12)),
                  child: Text("Simpan Perubahan", style: TextStyle(color: context.cardBg)),
                ),
              )
            ],
          ),
  ...
  Future<void> _saveLandSettings() async {
    ...
    if (success) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Berhasil menyimpan perubahan")));
      _loadData();
    } else {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Gagal menyimpan"), backgroundColor: Colors.red));
      setState(() => _isLoading = false);
    }
  }
}